...
class dataset(torch.utils.data.Dataset):
    ...
    def collate(self, batch):
        ...
        if self.args.dynamic_batching:
            if out['y'][0].ndim == 2:
                max_feature_dim = max(item.size(1) for item in out['y'])
                padded_y = []
                for item in out['y']:
                    padding = max_feature_dim - item.size(1)
                    padded_item = torch.nn.functional.pad(item, (0, padding), value=self.args.audio_pad_token)
                    padded_y.append(padded_item)
                res['y'] = torch.nn.utils.rnn.pad_sequence(
                    padded_y, batch_first=False, padding_value=self.args.audio_pad_token
                )
                res['y'] = res['y'].permute(1, 2, 0)  # T x B x K -> B x K x T
            elif out['y'][0].ndim == 1:
                res['y'] = torch.nn.utils.rnn.pad_sequence(
                    out['y'], batch_first=True, padding_value=self.args.audio_pad_token
                )
            else:
                raise ValueError(f"Unexpected shape for 'y': {out['y'][0].shape}")
        else:
            max_feature_dim = max(item.size(1) for item in out['y'])
            padded_y = []
            for item in out['y']:
                ...
                padded_y.append(padded_item)
            res['y'] = torch.nn.utils.rnn.pad_sequence(
                padded_y, batch_first=True, padding_value=self.args.audio_pad_token
            )
