...
#from steps.waveform_utils import WaveformUtils
from steps.waveform_utils import InferenceDecodeWrapper
...
class Trainer:
    ...
    #       self.train_sampler.set_epoch(self.progress['epoch'])
            for i, batch in enumerate(self.train_loader):
                ...
                for j in range(self.args.gradient_accumulation_steps):
                    ...
                    #ground_truth_tokens = cur_batch["y"].to(self.device) # Assuming cur_batch["y"] contains ground truth tokens
                    with torch.cuda.amp.autocast(dtype=torch.float16 if self.args.precision=="float16" else torch.float32):
                        ...
                        if out == None:
                            continue
                    # Decode the ground truth tokens
                    try:
                        # Prepare for decode
                       # concat_frames = ground_truth_tokens.permute(0, 2, 1)  # [B, K, T] -> [B, T, K]
                        gen_frames = out["tokens"].permute(0, 2, 1)  # Ensure predicted tokens are reshaped
                      #  print("Shape after permute:", gen_frames.shape)
                        # Decode using audio_tokenizer
                     #   concat_sample = self.audio_tokenizer.decode([(concat_frames, None)])  # Ground truth
                        gen_sample = self.audio_tokenizer.decode([(gen_frames, None)])  # Predicted

                        #print("Decoded Ground Truth Waveform Shape:", concat_sample.shape)
                        print("Decoded Predicted Waveform Shape:", gen_sample.shape)

                    except Exception as e:
                        print(f"Error during decoding: {e}")
                        print("Skipping this batch due to decode issues.")
                        continue
    ...
    def _setup_dataloader(self):
        ...
        if self.args.dynamic_batching:
            ...
            train_sampler = DistributedSampler(train_dataset, num_replicas=self.world_size, rank=self.rank, shuffle=True, seed=self.args.seed, drop_last=True)
       ...
       #     train_sampler.set_epoch_resume(self.progress['epoch'], self.progress['cur_step'])
        ...
        if self.args.dynamic_batching:
            ...
            train_loader = torch.utils.data.DataLoader(train_dataset, 
                            ...
                            collate_fn=train_dataset.collate, persistent_workers=False
                            )
            valid_loader = torch.utils.data.DataLoader(val_dataset, 
                            batch_size=self.args.batch_size//self.world_size, sampler=valid_sampler,
                            num_workers=self.args.num_workers//self.world_size,
                            collate_fn=val_dataset.collate, persistent_workers=False
                            )
    ...
    def _setup_models(self):
        ...
        if self.progress['step'] > 1:
            ...
            try:
                model.load_state_dict(bundle['model'], strict=False)  # Allow missing keys
